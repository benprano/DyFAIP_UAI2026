...
class ModelTrainer:
    ...
    def evaluate_model(self, model_class: type, model_path: str,
        ...
        return (np.hstack(real_inputs), np.hstack(imputed_inputs), np.vstack(freq_weights),
                ...
                np.vstack(uncertainty_weights),np.vstack(imp_imputs_batch), np.vstack(all_targets),
    ...
    def inference(self, model_class: type, model_path: str,
        ...
        (fin_inputs_i, inputs_outputs_i, freq_weights, h_bar_weights,
         ...
         gate_imputs_weights,uncertainty_weights,imp_imputs_batch,
         y_pred, y_true)= self.evaluate_model(
    ...
    def _final_evaluation(self, model_class: type, model_path: str,
       ...
       # ================================================================
        ...
        if self.is_classification:
            binary_scores = self.metrics.compute_binary_metrics([test_targets], [test_outputs])
            ...
            f1_scores = [(best_threshold, best_f1)]
            ...
            self.save_importances_with_json(features_weights, save_path, f'importance_manifolds_{model_name}')
            save_dict = {}
            for idx, imp_dict in enumerate(features_weights):
                key = f"layer_{imp_dict['layer']}_dir_{imp_dict['direction']}"
                save_dict[key] = imp_dict['importance']

            # Save
            np.savez(os.path.join(save_path,
                                  f'importance_manifolds_{model_name}.npz'), **save_dict)
            np.savez(
